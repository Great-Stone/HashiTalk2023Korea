...
source "amazon-ebs" "ubuntu-focal-amd64" {
  region        = var.region
  source_ami    = data.amazon-ami.ubuntu-focal-amd64.id
  instance_type = "t3.micro"
  ssh_username  = "ubuntu"
  ami_name      = "packer_ubuntu_focal_nomad_client_{{timestamp}}_v${var.version}"
}
...
source "amazon-ebs" "ubuntu-focal-arm64" {
  region        = var.region
...
build {
  hcp_packer_registry {
    bucket_name = "ubuntu-nomad-client"
    description = <<EOT
